...
verify_checksum() {
    local file="$1" checksums_url="$2"
    local expected actual
    expected="$(curl -fsSL "$checksums_url" 2>/dev/null | grep "$(basename "$file")" | awk '{print $1}' | head -1)"
    if [[ -z "$expected" ]]; then
        log WARN "no checksum entry found for $(basename "$file") — installing unverified"
        return 0
    fi
    actual="$(sha256sum "$file" | awk '{print $1}')"
    if [[ "$expected" != "$actual" ]]; then
        log FAIL "checksum MISMATCH for $(basename "$file") — expected ${expected}, got ${actual}"
        return 1
    fi
    log OK "checksum verified for $(basename "$file")"
    return 0
}

install_github_release_tool() {
    # install_github_release_tool <installed_name> <owner/repo> <asset_regex> <zip|targz|gz> [inner_binary_name]
    ...
    asset_url="$(echo "$json" | jq -r --arg p "$pattern" '[.assets[] | select(.name | test($p))][0].browser_download_url // empty')"
    ...
    if [[ -n "$checksum_url" ]]; then
        if ! verify_checksum "$tmp" "$checksum_url"; then
            rm -f "$tmp"; rm -rf "$extract_dir"
            FAILED_TOOLS+=("$name")
            return 1
        fi
    else
        log WARN "${name} — upstream published no checksums file, installing unverified"
    fi
...
install_github_release_tool "rustscan"  "RustScan/RustScan"          "linux.*\.tar\.gz$"     targz
...
if pipx install "git+https://github.com/Pennyw0rth/NetExec" >/tmp/netexec.log 2>&1; then
    log OK "netexec (nxc) installed"
    INSTALLED_TOOLS+=("netexec")
else
    log FAIL "netexec install failed — see /tmp/netexec.log"
