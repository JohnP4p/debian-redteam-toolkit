...
if ! apt-get install -y -qq --no-install-recommends \
    ...
    build-essential jq pipx python3-venv ufw unattended-upgrades libcap2-bin \
...
configure_firewall() {
    if is_qubes_template; then
        # Qubes enforces network filtering upstream, at the sys-firewall qube, AND
        # via its own qubes-firewall.service running locally inside every qube
        # (DNS redirection, anti-spoofing nftables rules). ufw --force enable takes
        # over iptables/nftables management wholesale, which is very likely to
        # flush and replace those local Qubes rules rather than coexist with them —
        # almost certainly the actual cause of "internet stopped working entirely,
        # even with a NetVM attached" after an earlier run of this script. Qubes
        # already provides the security property ufw exists for, at a layer this
        # script has no business duplicating. Skipped entirely here, not weakened.
        log INFO "Qubes template detected — skipping ufw. Qubes already enforces network filtering at sys-firewall + its own local qubes-firewall.service; running ufw here duplicates that and can conflict with it (this is the leading suspect if internet access broke after a previous run)."
        return 0
    fi
    ...
    if [[ "$ENABLE_SSH" == "1" ]]; then
        ...
        log INFO "No inbound ports opened (default)."
    fi
...
QUBES_TPL=0
if is_qubes_template; then QUBES_TPL=1; fi
...
} > /etc/sysctl.d/99-hacklab-hardening.conf

if [[ "$QUBES_TPL" == "1" ]]; then
    log INFO "Qubes template detected — skipping strict rp_filter. Qubes already does anti-spoofing at the sys-firewall layer (a dedicated nftables antispoof chain), and strict reverse-path filtering is a known cause of dropped traffic in exactly this kind of multi-hop NAT topology (AppVM -> sys-firewall -> sys-net)."
fi
...
if [[ ! -x "${BIN_DIR}/sliver-server" ]]; then
    if curl -fsSL https://sliver.sh/install | bash -s -- --no-client >/tmp/sliver-install.log 2>&1; then
        ...
        # Binding a listener to a port below 1024 (443/80 — the natural choice for
        # an HTTPS-mimicking C2 profile) needs root on Linux by default. Grant just
        # that one capability to the binary instead of requiring redteam to run the
        # whole thing as root — this is almost certainly why it "only worked as root".
        if command -v setcap >/dev/null 2>&1; then
            setcap 'cap_net_bind_service=+ep' "${BIN_DIR}/sliver-server" 2>/dev/null \
                && log OK "sliver-server granted CAP_NET_BIND_SERVICE — can bind ports <1024 without root" \
                || log WARN "setcap on sliver-server failed — binding ports <1024 will still need root; ports >=1024 work fine as redteam either way"
        fi
