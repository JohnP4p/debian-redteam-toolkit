...
DRY RUN — nothing has been changed. With the current configuration this would:
  ...
  Recon/web:      naabu, subfinder, httpx, katana, nuclei, ffuf, rustscan, nmap
...
install_github_release_tool() {
    ...
    asset_name="$(echo "$json" | jq -r --arg p "$pattern" '[.assets[] | select(.name | test($p))][0].name // empty')"
    ...
    asset_digest="$(echo "$json" | jq -r --arg p "$pattern" '[.assets[] | select(.name | test($p))][0].digest // empty' | sed 's/^sha256://')"
